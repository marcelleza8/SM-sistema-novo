...
export function useCronHumanizer() {
   ...
   *
   * @param cronExpression - A expressão CRON no formato "minuto hora dia-do-mês mês dia-da-semana".
   * Exemplo: "*\/15 8-18 * * 1-5"
   *
   * @returns Texto humanizado descrevendo a expressão CRON.
  ...
  const humanizeCron = (cronExpression: string): string => {
     ...
     *
     * @param field - Valor do campo (exemplo: "*", "1-5", "*\/15").
     * @param type - Tipo do campo (exemplo: "minuto", "hora").
     * @returns Texto humanizado para o campo especificado.
     */
    const describeField = (field: string, type: string): string => {
   ...
   *
   * @param humanizedText - Texto humanizado no formato gerado por `humanizeCron`.
   * Exemplo: "Minuto: a cada 15 minutos começando de 0, Hora: de 8 até 18, Dia do mês: qualquer dia do mês, Mês: qualquer mês, Dia da semana: de segunda-feira até sexta-feira"
   *
   * @returns A expressão CRON correspondente.
   * Exemplo: "*\/15 8-18 * * 1-5"
   */
  const parseHumanizedToCron = (humanizedText: string): string => {
    /**
     * Converte uma descrição humanizada de um campo para o formato CRON.
     *
     * @param text - Texto humanizado para o campo.
     * @param type - Tipo do campo (exemplo: "minuto", "mês").
     * @param map - (Opcional) Mapa de nomes para valores numéricos (exemplo: meses ou dias da semana).
     * @returns Valor do campo no formato CRON.
     */
    const parseField = (text: string, type: string, map?: string[]): string => {
